...
function WrittenNoteContainer({notes, jumpToHighlightArea, setViewMode, setSelectedNote, userEthAddress, setReload, currentPage}: props) {
    const { Moralis } = useMoralis();
    const user = Moralis.User.current();
    const transferProcessor = useWeb3Transfer();
    const currentNote = notes.filter((note) => {
        return ((note.highlightAreas[0].pageIndex === currentPage) || (note.highlightAreas[0].pageIndex === (currentPage - 1)) || (note.highlightAreas[0].pageIndex === (currentPage + 1)));
    })
    return (
        <>
                  ...
                  <div className="note-content">
                    ...
                    <div className="informationContainer">
                      ...
                      {note.buyers.includes(user?.get("username")) ? <button className="buy-button" onClick={() => {
                        ...
                        await buyNote(Moralis, transferProcessor, {
                          noteHash: noteHash,
